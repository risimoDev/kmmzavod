  ...
  async generateScript(opts: GenerateScriptOptions): Promise<ScriptGenerationResult> {
    ...
    if (isMock) {
      logger.info(
        ...
        'OpenRouter (orchestrator): Key not set or mock mode, using native product heuristic generator with Fish Audio emotion tags',
      );
      if (isFitMode) {
...
Ответ верни СТРОГО в формате JSON:
{
  ...
  "captions": [
    ...
    try {
      ...
      logger.warn({ err: errMsg }, 'OpenRouter (orchestrator) failed, using native product heuristic script generator');
      if (isFitMode) {
        return generateMockFitScript({
          currentScript: opts.currentScript!,
          topic: effectiveTopic,
