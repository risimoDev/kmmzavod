...
function generateMockScript(opts: {
  ...
  if (opts.style === 'sales') {
    ...
    body = `[confident] Мы протестировали десятки вариантов и нашли решение, которое экономит ваше время и бюджет уже с первого дня использования. [excited] Этот подход уже помог сотням людей добиться потрясающих результатов без лишних усилий. [confident] Главное следовать инструкции и применять всё на практике прямо сейчас.`;
    ...
    body = `[excited] То, что раньше казалось невозможным, теперь доступно каждому за считанные секунды. [confident] Смотрите, как это работает на практике. [surprised] Просто невероятный объем и качество без единого сложного движения. [confident] Всё продумано до мельчайших деталей для идеального результата каждый раз.`;
    cta = `[urgent] Ставьте лайк и делитесь этим видео с друзьями прямо сейчас!`;
  } else {
    if (sec > 30) {
      body += ` [excited] Обратите внимание на каждую деталь, ведь именно в мелочах кроется настоящий профессиональный секрет. [confident] Попробуйте этот способ и убедитесь сами уже сегодня.`;
    }
  }
...
export class OpenRouterService {
  async generateScript(opts: {
    ...
    // Russian speech rate in dynamic short videos: ~2.0 - 2.15 words per second
    ...
    if (isMock) {
      logger.info(
        { topic: opts.topic, style, seconds, targetWords },
...
- Общий объем произносимых слов (без учета эмоций в скобках): СТРОГО от ${minWords} до ${maxWords} слов (в среднем ${targetWords} слов).
- Если слов будет меньше ${minWords}, в конце видео образуется пустота и немота. Если слов будет больше ${maxWords}, голос не успеет прозвучать.
- Рассчитывай тайминг:
  * 0–3 сек: хук (первая фраза-крючок, 6–8 слов)
  * 3–${Math.max(4, seconds - 5)} сек: основная динамичная часть (подача фактов, сути, пользы, интриги)
  * Последние 4–5 сек: призыв к действию (CTA)
...
ПРАВИЛА ОЗВУЧКИ:
1. ХУК В ПЕРВЫЕ 3 СЕКУНДЫ: мгновенно цепляет внимание зрителя с тегом эмоции (например, "[surprised] Вы совершаете эту ошибку каждый день!").
