...
- ✅ **Фиксы раскадровки (2026-07-03, по багрепорту «один клип и без превью»)**:
  убран MIN_CLIP_GAP (резал соседние окна: 60с/30с → 1 клип; теперь чистый
  overlap-чек + клэмп после снапа); pHash-дедуп только для немых клипов (кадры
  «говорящей головы» одинаковы во все моменты — схлопывал всё в 1); превью
  реализованы e2e: editor заполняет `thumb_b64` (кадр середины клипа, 360px),
  editor-analyze.worker грузит в MinIO `.../thumbs/clip_NNN.jpg` → `thumbnailKey`
  и вычищает base64 из edl; мёртвая зона длительности 0.5T–T закрыта.
