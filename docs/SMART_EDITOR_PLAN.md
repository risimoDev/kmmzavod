...
- ✅ **Фикс mix + редактируемая раскадровка (2026-07-03, багрепорт «4 секунды и без
  субтитров»)**: `build_mix` теперь субдивидит длинные спаны на чанки ≤4с и добирает
  до цели (одна сцена без битов давала ОДИН 4с кусок — потому «монтаж 4 секунды»);
  порядок хронологический. Субтитры стали **данными EDL**: `map_clip_subtitles`
  проецирует транскрипт на выходной таймлайн (с компенсацией xfade 0.35с/стык),
  клип несёт `subtitles[]` → UI редактирует (границы фраз + текст + добавить/удалить),
  рендер вжигает их как есть (`_lines_from_clip`, karaoke-тайминги синтезируются для
  ручных фраз) и больше НЕ зависит от Whisper в рантайме. PATCH клипа принимает
  `segments` (правка границ, сервер пересчитывает субтитры/сниппет из транскрипта
  источника) и `subtitles`. UI: панель «✎ правка» на карточке клипа + предупреждение,
  когда субтитры включены, а речь не распознана. Проверено: юнит (60с→10×4с=40с),
  e2e рендер 7-сегментного монтажа с ASS из EDL, tsc/next build зелёные.
